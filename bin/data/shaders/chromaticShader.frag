...
// 	vec4 newColor = texture(tex0, texCoordVarying);
...
vec3 render( vec2 uv )
{
    ...
    if ( uv.x > 0.7 && uv.y > 0.7 )
    {
		...
		// uv is a unit vector but OF needs full resolution sample coordinates
		// for the texture sampler    
    vec2 samplePosition = vec2(1.0 - uv.x, (1.0 - uv.y));
    // vec2 samplePosition = uv;

    // return srgb2lin(texture( tex0, uv ).rgb );
    return srgb2lin(texture( tex0, samplePosition ).rgb );
}
...
{	
    ...
    float max_distort_px = 100.0 * (sin(time*1000.0)+1.0)/2.0;
